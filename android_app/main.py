...
class DataManager:
    ...
    def add_note(self, title: str, content: str, category: str = "Personal", tags: List[str] = None):
        """Add a new note"""
        note = {
    ...
    def detect_code(self, content: str) -> bool:
        ...
        code_indicators = [
            ...
            "<?php", "#!/", "SELECT ", "INSERT ", "UPDATE ", "DELETE ", "CREATE TABLE"
        ]
...
class DashboardScreen(MDScreen):
    ...
    def build_dashboard(self):
        ...
        # Stats cards
        stats_layout = MDGridLayout(cols=2, spacing=dp(10), size_hint_y=None, height=dp(120))
        ...
        # Recent notes
        recent_notes = sorted(self.data_manager.data["notes"], 
...
class NotesScreen(MDScreen):
    ...
    def populate_notes_list(self):
        ...
        notes = sorted(self.data_manager.data["notes"], 
                      key=lambda x: x["modified"], reverse=True)
        
        for note in notes:
            note_item = TwoLineListItem(
                text=note["title"],
                secondary_text=f"{note['content'][:50]}..." if len(note['content']) > 50 else note['content'],
    ...
    def show_add_note_dialog(self, *args):
        ...
        self.add_note_dialog = MDDialog(
            ...
            buttons=[
                MDFlatButton(
                    text="CANCEL",
                    on_release=self.close_add_note_dialog
                ),
                MDRaisedButton(
                    text="ADD",
    ...
    def view_note(self, note_id: int):
        ...
        note = next((n for n in self.data_manager.data["notes"] if n["id"] == note_id), None)
        if note:
            content = MDBoxLayout(
                orientation="vertical",
                spacing=dp(10),
            ...
            content.add_widget(MDLabel(text=f"Title: {note['title']}", size_hint_y=None, height=dp(40)))
            content.add_widget(MDLabel(text=f"Category: {note['category']}", size_hint_y=None, height=dp(40)))
            content.add_widget(MDLabel(text=f"Content:\n{note['content']}", text_size=(dp(250), None)))
...
class TasksScreen(MDScreen):
    ...
    def populate_tasks_list(self):
        ...
        tasks = sorted(self.data_manager.data["tasks"], 
        ...
        for task in tasks:
            ...
            checkbox = MDCheckbox(
                ...
                size_hint_x=None,
                width=dp(40),
                on_active=lambda checkbox, active, task_id=task['id']: 
    ...
    def view_task(self, task_id: int):
        ...
        if task:
            ...
            content.add_widget(MDLabel(text=f"Title: {task['title']}", size_hint_y=None, height=dp(40)))
            content.add_widget(MDLabel(text=f"Priority: {task['priority']}", size_hint_y=None, height=dp(40)))
            content.add_widget(MDLabel(text=f"Status: {'Completed' if task['completed'] else 'Pending'}", size_hint_y=None, height=dp(40)))
            content.add_widget(MDLabel(text=f"Description:\n{task['description']}", text_size=(dp(250), None)))
